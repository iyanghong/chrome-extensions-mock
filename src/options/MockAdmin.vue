<template>
  ...
  <MockMenuSelectModal ref='mockMenuSelectModalRef' :data='treeMockMenuData'
                       @select='handleSelectMenu'></MockMenuSelectModal>
</template>
...
async function handleLoad() {
  treeMockMenuData.value = [{
    ...
    parentId: '-1',
    parentPathText: '',
    children: await handler.sendBackgroundMessage('getTreeMockMenuData')
  }];
  ...
  return list.map((it: MenuTreeEntity) => {
    ...
    if (!it.expression) {
      option.suffix = () => {
        return h(NSpace, {}, {
          default: () => {
            const list = [h(NButton, {
              text: true, type: 'success', onClick: () => {
                console.log(`${it.name}=${it.id} create child`);
                mockMenuFormRef.value?.open({
                  name: '',
                  id: '',
                  parentId: it.id,
                  sort: 0,
                  parentPathText: it.name,
                  expression: null
                });
              }
            }, { default: () => '新增' })];
      ...
      option.suffix = () => {
        return h(NSpace, {}, {
          default: () => [
            ...
            h(NButton, {
              text: true, type: 'error', onClick: async () => {
                console.log(`${it.name}=${it.id} delete`);
                await handler.sendBackgroundMessage('removeMockMenu', it.id);
                await handleLoad();
              }
            }, { default: () => '删除' })
          ]
